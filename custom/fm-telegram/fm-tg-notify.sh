#!/usr/bin/env bash
# Outbound v3: send a message from the first mate via Telegram.
# Usage: fm-tg-notify.sh "<message>"                          -> captain DM (TG_CHAT_ID)
#        fm-tg-notify.sh -c <chat_id> "<message>"             -> specific chat (e.g. work group)
#        fm-tg-notify.sh -c <chat_id> -t <topic_id> "<message>" -> specific TOPIC in a forum group
set -eu
...
while :; do
  case "${1:-}" in
    -c) DEST="${2:?fm-tg-notify: -c needs a chat id}"; shift 2 ;;
    -t) TOPIC="${2:?fm-tg-notify: -t needs a topic id}"; shift 2 ;;
    *) break ;;
  esac
done
MSG="$*"
